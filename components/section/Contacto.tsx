...
export const Contacto: FC = () => {
  ...
  return (
    <Section ref={ref} id='contacto' paddingNone>
      <Box background='yellow' padding='50px 0'>
        <Title center color='#000' size={50} sizeXL={35}>
          Contacto
        </Title>
        <Text center weight={500}>
          Correo: guanuco43@gmail.com
        </Text>
      </Box>
      ...
      <Box padding='100px 0' width='min(700px, 95%)' center>
        ...
        {isSend && <p className={styles.contacto_succes}> Enviado!.👍🏼 </p>}
        <ButtonForm onClick={handleSendEmail}>
          {loader ? <Spinner /> : 'Enviar'}
        </ButtonForm>
      </Box>
    </Section>
  );
