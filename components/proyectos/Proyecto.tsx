...
export const Proyecto: FC<Props> = ({
  proyecto: { img, columns, repositorio, url, titulo, conocimientos },
  title,
  technologies,
}) => {
  return (
    <GridChildren
      ...
      className='relative'
      opacity={
        technologies.length > 0 && !compareArrays(conocimientos, technologies)
          ? '0.2'
          : undefined
      }
    >
      <Hover>
        {title ? (
          ...
          >
            <Flex center colCenter>
              <Title color='#000' sizeXM={12}>
                {titulo}
              </Title>
            </Flex>
          </Box>
        ) : (
          <ImageContainer src={img} cover radius='10px' />
        )}
            ...
            <Flex columnGap={10}>
              ...
              {url && (
                <Link href={url} target='_blank' rel='noopener noreferrer'>
                  <IconSize size={30} sizeXM={20}>
                    <FontAwesomeIcon icon={faGlobe} color='#000' />
                  </IconSize>
                </Link>
              )}
            </Flex>
        ...
        <Position bottom='2%' left='2%' className='translate'>
          <Flex wrap gap={5}>
            {conocimientos.map((c) => (
              <Tecnologia key={c} title={c} technologies={technologies} />
            ))}
          </Flex>
        </Position>
      </Hover>
